<template>
    <div class="font-sans antialiased" id="app">
  <nav class="w-screen fixed flex items-center justify-between flex-wrap bg-black p-6">
    <div class="flex items-center flex-no-shrink text-white mr-6">
      <img class="h-8 sm:h-20 mr-2 sm:mr-4 ml-0 sm:ml-12" src="./assets/logo.png" alt="">
      <span class="font-semibold text-xl tracking-tight"></span>
    </div>
    <div class="block sm:hidden">
      <button @click="toggle" class="flex items-center px-3 py-2 border rounded text-white border-white hover:text-textpurple hover:border-borderpurple">
        <svg class="fill-current h-3 w-3" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><title>Menu</title><path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z"/></svg>
      </button>
    </div>
    <div :class="open ? 'block': 'hidden'" class="w-full flex-grow sm:flex sm:items-center sm:w-auto">
      <div class="text-sm sm:flex-grow">

      </div>
      <div>
        <a href="#responsive-header" class=" no-underline block mt-4 sm:inline-block sm:mt-0 text-white sm:mx-8" @click="goto('life')">Life</a>
        <a href="#responsive-header" class="no-underline block mt-4 sm:inline-block sm:mt-0 text-white hover:text-white sm:mx-8" @click="goto('work')">Work Experience</a>
        <a href="#responsive-header" class="no-underline block mt-4 sm:inline-block sm:mt-0 text-white hover:text-white sm:mx-8" @click="goto('skills')">Skills</a>
        <a href="#responsive-header" class="no-underline block mt-4 sm:inline-block sm:mt-0 text-white hover:text-white sm:mx-8" @click="goto('projects')">Projects</a>
        <a href="#" class="no-underline inline-block text-sm px-4 py-2 leading-none border rounded text-white border-white hover:border-transparent hover:text-white hover:bg-black sm:ml-4 mt-4 sm:mt-0 mr-0 sm:mr-12">Contact</a>
      </div>
    </div>
  </nav>
      <main class="bg-backgrounddark w-fill h-fill py-4">

    <section class="mx-4 text-center mt-52 flex-col justify-center">
      <h1 class="mt-2 text-5xl sm:text-7xl text-white">Ilias Achahbar</h1>
      <p class="mt-2 text-lg text-white">Full Stack Web Developer</p>
    </section>

      <section class="text-center mt-40 flex-col justify-center bg-backgroundlightdark py-4 ">
      <h1 class="mt-2 text-5xl sm:text-7xl text-white" ref="life">Life</h1>
      <p class="mt-2 text-lg text-textpurple">...</p>




<div class="grid md:grid-cols-2">
      <div class="mt-4 p-6 md:mx-6 md:text-lg font-semibold">
        <div class="bg-backgroundpurple mx-8 text-white font-extrabold">
          <h2>#- - - Biography - - -#</h2>
        </div>

        <div class="text-white mt-10 md:mt-20">
          <p>Student in Leuven who lives in Mechelen, Belgium. I am in my graduation year at the UCLL studying Applied Information Technology.</p>
          <p class="mt-2">Currently fascinated by web development and all things around it.</p>
        </div>

      </div>
  <div class="m-auto">
    <img class="mx-auto w-4/5" src="./assets/bio.svg" alt="">
  </div>
</div>

    </section>

    <section class="text-center mt-40 flex-col justify-center bg-backgroundlightdark py-4">
      <h1 ref="work" class="mt-2 text-5xl sm:text-7xl text-white">Work Experience</h1>
      <p class="mt-2 text-lg text-textpurple">...</p>


<div class="grid sm:grid-cols-2 mt-8">
  <div class="m-auto">
    <img class="mx-auto w-4/5" src="./assets/work.svg" alt="">
  </div>
  <div>
<div class="bg-backgroundpurple mx-6 mt-10 text-white font-extrabold">
        <h2>#- - - Jobs - - -#</h2>
      </div>

      <div class="grid gap-4 md:grid-cols-2 justify-center mt-4 mx-6">
        <div class="h-auto w-auto p-4 bg-white font-semibold">
          <img class="mx-auto" src="./assets/emmaus.png" alt="">
          <h2 class="px-4">Intern • IT Department • Emmaüs vzw</h2>
        </div>

        <div class="h-auto w-auto bg-white p-4 font-semibold">
          <img class="mx-auto" src="./assets/emmaus.png" alt="">
          <h2 class="px-4">Administrative Work • IT Department • Emmaüs vzw</h2>
        </div>

        <div class="h-auto w-auto bg-white p-4 font-semibold">
          <img class="mx-auto" src="./assets/maarten.png" alt="">
          <h2 class="px-4">Cleaner • Cleaning Department • Sint-Maarten Mechelen</h2>
        </div>
  </div>
      </div>
      </div>
    </section>


      <section class="text-center mt-40 flex-col justify-center bg-backgroundlightdark py-8">
      <h1 class="mt-2 text-5xl sm:text-7xl text-white" ref="skills">Skills</h1>
      <p class="mt-2 text-lg text-textpurple">...</p>
    <div class="grid sm:grid-cols-2 gap-4">


<div>
      <div class="bg-backgroundpurple mt-10 mx-8 text-white font-extrabold">
        <h2># Programming Languages #</h2>
      </div>

      <div class="grid gap-4 md:grid-cols-4 justify-center mt-10 mx-8 font-semibold">

        <div class="h-auto w-auto border-b-8 border-jsyellow p-4">
          <img class="h-20 mx-auto" src="./assets/javascript-logo.png" alt="">
          <h2 class="text-white">JavaScript</h2>
          <div class="text-textpurple">
            <ul>
              <li>NodeJS</li>
              <li>VueJS</li>
              <li>Express</li>
            </ul>
          </div>

        </div>

        <div class="h-auto w-auto border-b-8 border-javared p-4">
          <img class="h-20 mx-auto" src="./assets/java-logo.png" alt="">
          <h2 class="text-white">Java</h2>
          <div class="text-textpurple">
            <ul>
              <li>Spring</li>
            </ul>
          </div>
        </div>

        <div class="h-auto w-auto border-b-8 border-csharppurple p-4">
          <img class="h-20 mx-auto" src="./assets/csharp.svg" alt="">
          <h2 class="text-white">C#</h2>
        </div>

        <div class="h-auto w-auto border-b-8 border-phppurple p-4">
          <img class="h-20 mx-auto" src="./assets/php-logo.png" alt="">
          <h2 class="text-white">PHP</h2>
        </div>

        <div class="h-auto w-auto border-b-8 border-cplusplusblue p-4">
          <img class="h-20 mx-auto" src="./assets/cplusplus-logo.png" alt="">
          <h2 class="text-white">C++</h2>
        </div>

        <div class="h-auto w-auto border-b-8 border-elixirpurple p-4">
          <img class="h-20 mx-auto" src="./assets/elixir-logo.png" alt="">
          <h2 class="text-white">Elixir</h2>
          <div class="text-textpurple">
            <ul>
              <li>Phoenix</li>
            </ul>
          </div>
        </div>

        <div class="h-auto w-auto border-b-8 border-cssblue p-4">
          <img class="h-20 mx-auto" src="./assets/css-logo.png" alt="">
          <h2 class="text-white">CSS</h2>
          <div class="text-textpurple">
            <ul>
              <li>Tailwindcss</li>
              <li>Vuetify</li>
            </ul>
          </div>
        </div>
      </div>

</div>

<div>

      <div class="bg-backgroundpurple mt-10 mx-8 text-white font-extrabold">
        <h2># Languages #</h2>
      </div>
      
      <div class="grid gap-4 grid-cols-1 justify-center mt-10 mx-20 font-semibold">

        <div class="h-auto w-auto border-b-8 border-white p-4">
          <h2 class="text-white">Dutch</h2>
        </div>

        <div class="h-auto w-auto border-b-8 border-white p-4">
          <h2 class="text-white">English</h2>
        </div>

        <div class="h-auto w-auto border-b-8 border-white p-4">
          <h2 class="text-white">French</h2>
        </div>

      </div>

</div>    
    </div>
    </section>

      <section class="text-center mt-40 flex-col justify-center bg-backgroundlightdark py-4 ">
      <h1 class="mt-2 text-5xl sm:text-7xl text-white" ref="projects">Projects</h1>
      <p class="mt-2 text-lg text-textpurple">...</p>




<div class="grid sm:grid-cols-4 mx-20 justify-center gap-20">
          <div class="h-auto w-auto border-b-8 border-white p-3">
            <h2 class="text-xl sm:text-2xl text-white">Computer Electronics Webshop</h2>
            <p class="text-textpurple">Complete webshop build with HTML, CSS and PHP</p>
            </div>
          <div class="h-auto w-auto border-b-8 border-white p-3">
            <h2 class="text-xl sm:text-2xl text-white">Payment System</h2>
            <p class="text-textpurple">A payment system for a store build with all the programming principles in JAVA</p>
            </div>
          <div class="h-auto w-auto border-b-8 border-white p-3">
            <h2 class="text-xl sm:text-2xl text-white">Gomoku Game</h2>
            <p class="text-textpurple">A small game made in C# using WPF</p>
            </div>
          <div class="h-auto w-auto border-b-8 border-white p-3">
            <h2 class="text-xl sm:text-2xl text-white">Online Animal Shelter with Webshop</h2>
            <p class="text-textpurple">Complete website made with the Phoenix framework using Elixir</p>
            </div>
          <div class="h-auto w-auto border-b-8 border-white p-3">
              <h2 class="text-xl sm:text-2xl text-white">Discord bot</h2>
            <p class="text-textpurple">Discord bot with small mining game. Made with Javascript, DiscordJS, NodeJS, MySQL</p>
            </div>
            <div class="h-auto w-auto border-b-8 border-white p-3">
              <h2 class="text-xl sm:text-2xl text-white">Expirimental Projects ...</h2>
            <p class="text-textpurple">Couple of non-finished projects using the following stacks: MEVN or Vue/Spring/PostGresSQL</p>
            </div>
</div>
	
    </section>
    

  </main>
  </div>
</template>

<script>
// import Hero from './components/Hero.vue'

export default {
  name: 'App',
  components: {
    // Hero
  },
  data: function(){
    return {
      open: false,
    }
  },
  methods: {
    toggle() {
      this.open = !this.open
    },
    
    goto(refName) {
      var element = this.$refs[refName];
      console.log(element);
      var top = element.offsetTop;
      
      window.scrollTo(0, top - 299);
    }
  }
}
</script>

<style>

</style>
